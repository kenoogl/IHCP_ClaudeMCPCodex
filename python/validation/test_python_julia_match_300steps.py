...
def main():
    ...
    # 一時ファイル名
    temp_filename = "temp_python_300steps_q.npy"
    ...
    # 個別に保存（.npy形式、非圧縮）
    q_path = output_dir / "python_300steps_q.npy"
    T_path = output_dir / "python_300steps_T.npy"

    np.save(q_path, q_result)
    np.save(T_path, T_verify)

    # メタデータ付き圧縮版も保存
    npz_path = output_dir / "python_300steps.npz"
    np.savez_compressed(
        npz_path,
        # 入力データ
        Y_obs=Y_obs,
    ...
    print(f"  温度誤差（最大）: {max_error:.4e} K")
    print(f"\n次のステップ:")
    print(f"  Julia版を実行: julia julia/examples/test_python_julia_match_300steps.jl")
    print("="*80)
