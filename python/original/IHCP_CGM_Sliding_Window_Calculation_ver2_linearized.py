...
def multiple_time_step_solver_Sensitivity(T_base, dT_initial, q_surface, rho, cp_coeffs, k_coeffs,
                                          dx, dy, dz, dz_b, dz_t, dt, rtol, maxiter):
    """
    Sensitivity solver that linearizes the conduction equation around a base temperature field.

    Parameters
    ----------
    T_base : numpy.ndarray
        Base temperature history from the direct solver, shape (nt, ni, nj, nk).
    dT_initial : numpy.ndarray
        Initial perturbation field at t=0, shape (ni, nj, nk).
    q_surface : numpy.ndarray
        Surface heat-flux direction (e.g., CGM search direction), shape (nt-1, ni, nj).
    """
    nt = T_base.shape[0]
    ni, nj, nk = dT_initial.shape

    if q_surface.shape != (nt - 1, ni, nj):
        raise ValueError(f"q_surface shape mismatch: expected {(nt - 1, ni, nj)}, got {q_surface.shape}")

    dT_all = np.empty((nt, ni, nj, nk))
    dT_all[0] = dT_initial

    x0 = dT_initial.ravel(order="F").copy()

    for t in range(1, nt):
        base_T = T_base[t-1]
        cp_base, k_base = thermal_properties_calculator(base_T, cp_coeffs, k_coeffs)

        T_prev = dT_all[t-1]
        a_w, a_e, a_s, a_n, a_b, a_t, a_p, b = coeffs_and_rhs_building_DHCP(
            T_prev, q_surface[t-1], rho, cp_base, k_base, dx, dy, dz, dz_b, dz_t, dt
        )
        ...
        if info > 0:
            print(f"[Sens警告][t={t}] Krylov法が収束しませんでした (info={info})")
        elif info < 0:
            print(f"[Sensエラー][t={t}] Krylov法の入力が不正です (info={info})")

        dT_all[t] = x.reshape((ni, nj, nk), order="F")
        x0 = x.copy()
...
def global_CGM_time(T_init, Y_obs, q_init, dx, dy, dz, dz_b, dz_t, dt,
    ...
    for it in range(CGM_iteration):
        ...
        dT = multiple_time_step_solver_Sensitivity(
                T_cal, dT_init, p_n, rho, cp_coeffs, k_coeffs,
                dx, dy, dz, dz_b, dz_t, dt,
                rtol = 1e-8, maxiter = 20000
                )
