...
1. **DHCPのみで予備確認**  
   - まず `run_sliding_window.jl --dry-run` で窓の分割を把握し、候補 window/overlap を選ぶ。  
   - `run_10steps_fullsize_test.jl` など DHCP テスト用スクリプトで、候補ウィンドウ長に対応する `nt` を変えて **反復数と1窓当たりの計算時間が極端に増えない範囲** を絞り込む。  
   - これにより、感度・随伴ソルバーまで進んで失敗するような「悪い」ウィンドウ設定を事前に除外できる。  
2. **候補設定**  
   - 上記で残したウィンドウ長（例: 50, 71, 100）と、20〜30% 程度のオーバーラップ比率（例: 0.2, 0.3）を `tune_sliding_window.jl` の引数にする。  
3. **探索実行**  
   ```bash
