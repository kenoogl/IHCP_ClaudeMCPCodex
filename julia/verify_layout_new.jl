...
新レイアウト版(tuning2)の検証スクリプト

配列形状:
- Y_obs: (ni, nj, nt)  # 新レイアウト
- T_init: (ni, nj, nk)
- q_init: (ni, nj, nt-1)  # 新レイアウト
"""
...
# 観測データ（ダミー、新レイアウト）
Y_obs = zeros(Float64, ni, nj, nt)  # (ni, nj, nt) 新レイアウト
for t in 1:nt
  Y_obs[:, :, t] .= 300.0 + 0.1 * t
end
...
println("=== 新レイアウト版検証開始 ===")
println("配列形状:")
println("  Y_obs: $(size(Y_obs))  # 期待: (ni=$ni, nj=$nj, nt=$nt)")
...
# 結果保存
output_file = "layout_new_result.npz"
