...
# 本測定（プロファイリング）
Y_obs = zeros(Float64, nt, ni, nj)  # 旧レイアウト
for t in 1:nt
  Y_obs[t, :, :] .= 300.0 + 0.1 * t
end
...
# プロファイル結果を保存
println("\nプロファイル結果を profile_old.txt に保存中...")
open("profile_old.txt", "w") do io
...
println("\n" * "="^70)
println("プロファイル結果保存完了: profile_old.txt")
println("="^70)
