...
実行方法:
  JULIA_NUM_THREADS=1 julia julia/examples/test_python_julia_match_100steps.jl
"""
...
function main()
    ...
    # 個別に保存（.npy形式、非圧縮）
    q_path = joinpath(output_dir, "julia_100steps_q.npy")
    T_path = joinpath(output_dir, "julia_100steps_T.npy")

    npzwrite(q_path, q_result)
    npzwrite(T_path, T_verify)

    # メタデータ付き圧縮版も保存
    npz_path = joinpath(output_dir, "julia_100steps.npz")
    ...
    @printf("  温度誤差（最大）: %.4e K\n", max_error)
    println("\n次のステップ:")
    println("  結果比較: python python/validation/compare_python_julia_100steps.py")
    println("="^80)
