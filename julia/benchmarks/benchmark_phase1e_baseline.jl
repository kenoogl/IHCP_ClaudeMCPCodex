#!/usr/bin/env julia
"""
Phase 1-E Baseline Benchmark (adaptive_tol = false)

Phase 1-Bの最適パラメータ（dhcp_extrapolation=:quadratic, adjoint_residual_scale=0.5）を使用した
ベースラインベンチマーク。適応的収束判定は無効（adaptive_tol=false）。
"""
...
const RESULT_PATH = joinpath(PROJECT_ROOT, "shared", "results", "phase1e_baseline.npz")
...
function main()
  println("="^80)
  println("Phase 1-E Baseline Benchmark (adaptive_tol = false)")
  println("="^80)
  ...
  println("  Phase 1-B最適パラメータ適用 + adaptive_tol=false（ベースライン）")
  solve_start = time()

  # Phase 1-B最適パラメータ + adaptive_tol=false（ベースライン）
  cgm_params = (
    max_iter = 1,
    sigma = 1.8,
    rtol_dhcp = 1.0e-6,
    ...
    # Phase 1-E: 適応的収束判定（無効）
    adaptive_tol = false                       # ベースライン：適応的収束判定なし
  )
