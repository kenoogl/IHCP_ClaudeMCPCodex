#!/usr/bin/env julia
"""
Phase 1-E Adaptive Tolerance Parameter Tuning Benchmark

Phase 1-Eのパラメータ調整版：
- kappa_theta: 0.2 → 0.1（より保守的な緩和）
- warmup_steps: 3 → 4（より長いwarmup期間）

目的：一部ステップでの反復回数増加（悪化）を抑制
"""
...
const RESULT_PATH = joinpath(PROJECT_ROOT, "shared", "results", "phase1e_tuned.npz")
...
function main()
  println("="^80)
  println("Phase 1-E Parameter Tuning Benchmark")
  println("="^80)
  println("調整パラメータ:")
  println("  - kappa_theta: 0.2 → 0.1（より保守的）")
  println("  - warmup_steps: 3 → 4（より長いwarmup）")
  ...
  println("  Phase 1-B最適パラメータ + Phase 1-E調整版")
  solve_start = time()

  # Phase 1-B最適パラメータ + Phase 1-E調整版パラメータ
  cgm_params = (
    max_iter = 1,
    sigma = 1.8,
    rtol_dhcp = 1.0e-6,
    ...
    kappa_theta = 0.1,                         # ★調整: 0.2 → 0.1（より保守的）
    warmup_steps = 4                           # ★調整: 3 → 4（より長いwarmup）
  )
