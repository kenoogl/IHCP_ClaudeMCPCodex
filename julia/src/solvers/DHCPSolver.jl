...
"""
    calRHS!(wk, HF, dx, dy, Δt, ΔZ, z_range, qsrf, q_dist, ρ, par)
...
"""
    solve_dhcp!(T_initial, q_surface, wk, nt, ρ, cp_coeffs, k_coeffs, dx, dy, Z, ΔZ, dt;
                rtol=1e-6, maxiter=20000, verbose=false, par="sequential") -> T_all

DHCP（直接熱伝導問題）ソルバー

各時間ステップで、前ステップ温度から熱物性値計算と反復求解を実行

# 引数
- `T_initial::Array{Float64,3}`: 初期温度場 (ni, nj, nk) [K]
- `q_surface::Array{Float64,3}`: 表面熱流束 (ni, nj, nt-1) [W/m²]
