...
# basesize最適化測定スクリプト（デフォルトサイズ）
# 問題サイズ: 80×100×20 (N=160,000)
# basesizeパラメータ: 400, 800, 1000, 2000
# スレッド数: 4

echo "=== basesize最適化測定（デフォルトサイズ80×100×20） ==="
echo "日時: $(date)"
echo ""
...
for basesize in 400 800 1000 2000; do
  ...
  JULIA_NUM_THREADS=4 julia --project=julia julia/scripts/test_dhcp_solver.jl \
    --ni 80 --nj 100 --nk 20 --nt 10 --synthetic --basesize $basesize \
    2>&1 | tee shared/results/basesize_optimization/size_80x100x20_basesize_${basesize}.log
...
echo ""
echo "結果ファイル一覧:"
ls -lh shared/results/basesize_optimization/size_80x100x20*.log
