...
2. **ファイルパスの存在を確認**
   ```bash
   ls <実行するファイルのパス>  # ファイルが存在するか確認
   ```

3. **正しいパスで実行**
   - プロジェクトルート: `/Users/Daily/Development/IHCP/TrialClaudeMCPCodex`
   - 常にプロジェクトルートからの相対パスで実行
   - 例: `python3 python/scripts/run_sliding_window.py`（✅正しい）
   - 例: `python3 python/python/scripts/run_sliding_window.py`（❌間違い）

4. **同じ間違いを繰り返さない**
   - パスエラーが出たら、必ず `pwd` と `ls` で確認してから再実行
   - 推測でパスを修正しない
   - 実態を確認してから実行する
...
### 禁止事項
1. **推定値・仮定値の使用禁止**
   - 「3回目は2回目と同等と仮定」等の推定値でレポート作成禁止
   - 実測データが不完全な場合はレポート作成を中断
   - 「推定」「仮定」「想定」等の曖昧な表現を使わない
...
1. **実行完了確認**
   ```bash
   # 必ず実行して確認
   grep "Total runtime:" <logfile>  # 完了確認
   ls -lh <result_file>              # ファイル確認
   tail -20 <logfile>                # エラー確認
   ```
...
3. **レポート作成基準**
   - **実測データのみ使用**: 100%実測された完全なデータ
   - **完了確認済み**: "Total runtime:"等の完了マーカー確認済み
